...
export default function ItemDetailsDrawer({ item, currentUser, isOpen, onClose }: ItemDetailsDrawerProps) {
	
	const PriorityIcon = item?.priority ? prioritiesMap[item.priority].el : prioritiesMap[1].el;
	const userGiverName = item?.reservations.anonymous_giver ? 'alguém (anônimo)' : item?.reservations?.profile?.username
                ...
                <div className="overflow-y-auto overflow-x-hidden">
                    ...
                    <div className="p-4 w-full flex flex-col justify-center items-center gap-4">
                        <div className='max-w-xl w-full flex flex-col gap-2 items-center justify-center text-center'>
                            <div className='w-full flex bg-gray-100 px-4 pt-2 pb-4 rounded-lg flex-col gap-2 items-center justify-center'>
                                <h3 className='text-gray-500 font-semibold'>Observações</h3>
                                <p className={item.notes ? 'text-gray-500' : 'text-gray-400'}>{item.notes || 'Sem informações...'}</p>
                            </div>
                            <div className='w-full my-8 fle'>
								...
								{item.reservations && <div className='flex justify-center my-4 p-4 items-center gap-2 bg-green-50 rounded-lg'>
									{item.reservations.user_id &&
									currentUser.id !== item.reservations.user_id ?
									<MotionDiv 
										initial={{ opacity: 0, y: 10 }}
										animate={{ opacity: 1, y: 0 }}
										transition={{ duration: 0.5 }}
										className='flex items-center gap-1 flex-wrap justify-center w-full'>
										<p className='text-gray-600  text'>Este item já foi reservado por</p>
										<p className='text-gray-600 font-semibold truncate '>{userGiverName}</p>
									</MotionDiv>
									:
									<MotionDiv 
										initial={{ opacity: 0, y: 10 }}
										...
										<Button onClick={onClose} variant='outlined' className='w-full border-red-500 text-red-500 hover:bg-red-200 bg-red-100 rounded-full'>Remover reserva</Button>
									</MotionDiv>
									}
								</div>}
