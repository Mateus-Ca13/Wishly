...
type OwnerItemViewerDrawerProps = {
...
export default function OwnerItemViewerDrawer({ item, isOpen, onClose, onEditItem }: OwnerItemViewerDrawerProps) {
                            ...
                            <div className='w-full my-8'>
                                ...
                                <div className='flex flex-col gap-2 justify-center items-center w-full mt-8'>
                                    {item.link && item.link.length > 0 ?
                                        <Link href={item.link} target="_blank" className='w-full'>
                                            <Button className='w-full rounded-full py-4 flex flex-col justify-center items-center' variant='contained'>
                                                ...
                                                <span className='truncate font-extralight text-sm text-primary-100 w-full text-center px-4'>{item.link}</span>
                                            </Button>
                                        </Link> :
                                    ...
                                    <Button
                                        onClick={() => {
                                            onClose()
                                            setTimeout(() => onEditItem(item, 'edit'), 300)
                                        }}
                                        className='w-full rounded-full flex justify-center py-4'
                                        variant='outlined'
                                    >
                                        <div className='flex items-center gap-2'>
