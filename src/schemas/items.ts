...
export const getRegisterOrEditItemSchema = (t: any) => z.object({
    name: z.string().min(3, t('nameInput.minLength')).max(100, t('nameInput.maxLength')).trim(),
    ...
    notes: z.string().max(500, t('notesInput.maxLength')).trim(),
    link: z.string().url(t('linkInput.invalid')).max(1000, t('linkInput.maxLength')).trim(),
})
